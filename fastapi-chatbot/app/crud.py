"""
CRUD functions for chat_sessions and chat_messages.
"""
...
def add_message(session_id: int, role: str, content: str, file_path: Optional[str] = None) -> dict:
    """UPDATED: Insert message along with optional file path pointer."""
    with get_conn() as conn:
        conn.autocommit = True
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO chat_messages (session_id, role, content, file_path)
                   VALUES (%s, %s, %s, %s)
                   RETURNING id, session_id, role, content, file_path, created_at;""",
                (session_id, role, content, file_path),
            )
            row = cur.fetchone()
...
def list_messages(session_id: int) -> list[dict]:
    """UPDATED: Retrieve messages including file_path column."""
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT id, session_id, role, content, file_path, created_at
                   FROM chat_messages
                   WHERE session_id = %s
